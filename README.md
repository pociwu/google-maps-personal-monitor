...
- 正常巡查每 6～8 小時；日期切換候選每 5～6 分鐘檢查一次，到期對象每 20～30 分鐘密集觀察。
...
- 貢獻者卡片保存並顯示頭像、在地嚮導等級、積分與下一級進度。
- 提供發文時間分析，分開呈現星期、四時段、24 小時、月份與發文間隔。
...
- `maps-monitor-dense.timer`：每 5～6 分鐘喚醒一次；同一到期對象每 20～30 分鐘密集觀察。
...
- Google 完整時間與窄區間推算的數量；推算時間只在整個證據區間不超過 1 小時時納入時段圖。
- 發文間隔中位數、IQR、接近中位數的比例與前後期一致性，以及保守的星期／時段偏向判斷。
- 圖表使用全部資料；逐筆明細為避免手機頁面過大，最多顯示最近 100 筆。

只有完整時間戳才算 Google 精確時間；總區間不超過 1 小時的資料在 24 小時圖中使用區間中點，頁面會保持「推算」標示。星期 × 2 小時圖採更保守規則：推算區間必須完整落在同一個台灣日期及同一個 2 小時格才納入。全部貢獻者合併圖只作總覽，不跨人物判定共同規律。單一貢獻者未達長期樣本門檻時，只顯示「初步偏向」並明確標示尚不能確認穩定規律。同一貢獻者同日的多則評論在規律判斷時會去重，圖表仍保留每一則資料。
...
日期引擎永久保存每次相對時間原文、解析單位、可能區間、可信度、時間主體及模型版本。公開 DOM 若提供完整時間戳，必須在相鄰兩輪一致才採用，中間缺漏不會被視為連續證據。相對時間會與首次看見時間交集；天與週需要相鄰數字切換並二次確認，月份與年份各需至少 3 個一致樣本完成模型校準。第一次切換仍不足 1 小時時會繼續預約下一次切換，不會因先取得粗略日期就停止。

頁面只有在實際證據區間總寬度不超過 1 小時時，才顯示到分鐘並標示對應誤差；例如區間總寬 50 分鐘會顯示中點及「± 25 分」。這代表已把發文時間限制在一小時內，不代表 Google 提供了精確分鐘。只有連續取得相同完整時間戳才顯示為 Google 精確時間。日期更新 Telegram 也只發送已確認的高可信結果。

一般巡查與密集巡查共用程序鎖。一般巡查會等待前一輪完成；密集巡查若剛好撞上一般巡查會正常略過並以成功狀態結束，因此不會再觸發 systemd 失敗通知。真正的服務失敗通知會附上來源 unit 名稱。

舊資料庫由新版第一次開啟前會備份至：

```text
state/backups/pre-schema-v8-*.sqlite3
```

遷移失敗會自動還原。舊推算日期保留為 `legacy_publish_date`。

schema v8 會先備份；新增貢獻者頭像、在地嚮導等級與積分進度欄位。schema v7
則會把升級當下的每則評論建立為第 1 版。之後文字、星等或
